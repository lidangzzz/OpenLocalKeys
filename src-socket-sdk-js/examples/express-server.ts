/**
 * Express.js Server Example using OpenLocalKeys Unix Socket SDK
 *
 * This example shows how to integrate OpenLocalKeys into a Node.js web application
 * using Unix sockets for faster, local-only communication.
 *
 * Prerequisites:
...
app.get('/health', (_req: Request, res: Response) => {
  res.json({
    status: 'ok',
    service: 'OpenLocalKeys Example Server (Unix Socket)',
    version: '1.0.0',
 ...
 *
 * This endpoint requests keys from the OpenLocalKeys Unix socket server.
  ...
  try {
    ...
    if (!status.running) {
      ...
      return res.status(503).json({
        ...
        message: 'Please start the OpenLocalKeys server: swift run OpenLocalKeys',
        status
      });
    }

    console.log('[/api/keys] Server is running, requesting keys...');
    ...
    // Check if it's an OpenLocalKeys error
      ...
      switch (error.code) {
        ...
        case ErrorCode.SocketNotFound:
          return res.status(503).json({
            ...
            message: 'Cannot connect to OpenLocalKeys server'
          });
...
app.get('/', (_req: Request, res: Response) => {
  res.send(`
    ...
    <html lang="en">
    <head>
        <title>OpenLocalKeys Socket SDK - Example Server</title>
        <style>
            body {
    ...
    <body>
        <div class="container">
            <h1>🔑 OpenLocalKeys Socket SDK</h1>
            <p>A TypeScript/JavaScript SDK for the OpenLocalKeys Unix socket server</p>
            <span class="badge">Unix Socket Transport</span>

            <h2>Available API Endpoints</h2>

            <div class="endpoint">
                <h3>POST /api/keys</h3>
                <p>Request API keys from OpenLocalKeys via Unix socket</p>
  ...
  -d '{"prompt": "Hello, world!", "model": "gpt-4"}'</pre>
            ...
            <p><em>Make sure the OpenLocalKeys Unix socket server is running first: <code>swift run OpenLocalKeys</code></em></p>
        </div>
    </body>
    </html>
  `);
...
app.listen(PORT, () => {
  ...
  console.log('⚠️  Make sure OpenLocalKeys Unix socket server is running:');
  console.log('   swift run OpenLocalKeys\n');
  console.log('📡 Using Unix socket transport (faster than HTTP!)');
});
